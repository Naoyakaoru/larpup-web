...
export default function RegisterPage() {
  const [form, setForm] = useState({ email: '', password: '', passwordConfirmation: '', nickname: '', gender: '' })
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await authApi.register(form.email, form.password, form.passwordConfirmation, form.nickname, form.gender)
    ...
    <div className="max-w-sm mx-auto mt-16">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="password" value={form.passwordConfirmation} onChange={set('passwordConfirmation')} required
            className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand" />
        </div>
