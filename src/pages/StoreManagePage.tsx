...
import { getStoreScriptVersions, updateStoreScriptVersion, createStoreScriptVersion } from '../api/stores'
...
function AddForm({ storeId, onAdded, onCancel }: { storeId: number; onAdded: (v: StoreScriptVersion) => void; onCancel: () => void }) {
    ...
    try {
      ...
      if (showNew) {
        version = await createStoreScriptVersion(storeId, {
          ...
          genres: newGenres, duration_override: Number(newDuration), price: Number(price),
    ...
    <form onSubmit={handleSubmit} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-surface-2">
          ...
          <div className="absolute z-10 w-full mt-1 bg-surface-2 border border-gray-200 rounded-md shadow-md overflow-hidden">
            {results.length === 0 ? (
              ...
              <ul className="max-h-48 overflow-y-auto divide-y divide-gray-100">
                ...
                <li>
                  <button type="button" onClick={() => { setShowNew(true); setDropdownOpen(false) }}
                    className="w-full text-left px-4 py-2.5 text-sm text-brand hover:bg-gray-50">+ 新增全新劇本</button>
                </li>
              </ul>
            )}
          </div>
      ...
      {showNew && (
        <div className="space-y-3 border border-brand/20 rounded-md p-3 bg-brand/5">
          ...
          <input type="text" value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="劇本名稱" required
          ...
          <select value={newDifficulty} onChange={e => setNewDifficulty(e.target.value)}
            className="w-full border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand">
            <option value="easy">入門</option><option value="medium">進階</option><option value="hard">燒腦</option>
          </select>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {!showNew && (
          <div>
            <label className="text-xs text-gray-500 mb-0.5 block">時長 override（選填，小時）</label>
      ...
      <div>
        ...
        <input type="text" value={versionName} onChange={e => setVersionName(e.target.value)} placeholder="如：標準版、體驗版"
          className="w-full border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand" />
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel} className="text-sm text-gray-500 hover:text-gray-700 px-3">取消</button>
      </div>
    </form>
  )
}
...
export default function StoreManagePage() {
  ...
  useEffect(() => {
    getStoreScriptVersions(storeId).then(setVersions).finally(() => setLoadingVersions(false))
  }, [storeId])

  async function toggleAvailable(v: StoreScriptVersion) {
    const updated = await updateStoreScriptVersion(storeId, v.id, { available: !v.available })
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="space-y-4">
        ...
        <section className="bg-surface border border-gray-200 rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to={`/stores/${id}/script_versions`} className="text-sm text-gray-400 hover:text-brand">
                查看全部 →
              </Link>
            </div>
          </div>

          {showAdd && <div className="mb-4"><AddForm storeId={storeId} onAdded={handleAdded} onCancel={() => setShowAdd(false)} /></div>}
                ...
                <li key={v.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {v.script.title}
                      {v.version_name && <span className="ml-1.5 text-xs text-gray-400">（{v.version_name}）</span>}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {DIFFICULTY_LABELS[v.script.difficulty as keyof typeof DIFFICULTY_LABELS]}・{v.script.total_slots} 人
                      {v.price != null && ` · $${v.price}`}
                    </p>
                  </div>
                  <button type="button" onClick={() => toggleAvailable(v)}
                    ...
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${v.available ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </li>
              ))}
              {versions.length > 5 && (
        ...
        <ComingSoonSection title="活動" description="查看與管理此店家的揪團活動" />
        <ComingSoonSection title="店家資訊" description="編輯店家名稱、簡介等基本資料" />
        <ComingSoonSection title="成員與權限" description="管理可操作此店家的帳號與角色" />
      </div>
    </div>
  )
}
