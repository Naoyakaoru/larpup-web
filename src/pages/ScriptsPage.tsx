          ...
          {scripts.map(script => (
            <Link key={script.id} to={`/scripts/${script.id}`} className="block bg-surface rounded-lg border border-gray-200 p-4 hover:border-brand-light hover:shadow-sm transition-all">
              ...
              <div className="text-xs text-gray-400">
                ...
                {script.duration != null && <span className="ml-1">・⏱ {script.duration}h</span>}
              </div>
              {script.description && (
...
function StoresTab() {
  ...
  if (stores.length === 0) return <div className="text-center text-gray-400 py-16">目前沒有店家資料</div>

  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {stores.map(store => (
        <div key={store.id} className="bg-surface rounded-lg border border-gray-200 p-4">
          <h2 className="font-semibold text-gray-900 text-sm mb-1">{store.name}</h2>
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-5 border-b border-gray-200">
        {([['scripts', '劇本'], ['stores', '店家']] as [Tab, string][]).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
