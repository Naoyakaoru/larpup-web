    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-surface rounded-lg border border-gray-200 overflow-hidden mb-4">
        ...
        <div className="p-6">
          ...
          <dl className={`grid gap-3 text-sm mb-4 ${script.duration != null ? 'grid-cols-3 sm:grid-cols-5' : 'grid-cols-2 sm:grid-cols-4'}`}>
            ...
            {script.duration != null && (
              <div className="text-center bg-gray-50 rounded-md py-2">
                ...
                <dd className="font-semibold text-gray-900">{script.duration}h</dd>
              </div>
            )}
          </dl>
            ...
            {versions.map(v => (
              <li key={v.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  ...
                  {v.version_name && (
                    <span className="ml-2 text-xs text-gray-400">{v.version_name}</span>
                  )}
                </div>
                <Link
                  to={`/events/new?script_version_id=${v.id}&script_id=${script.id}`}
                  state={{ store_name: v.store.name, version_name: v.version_name, price: v.price, duration: v.duration }}
